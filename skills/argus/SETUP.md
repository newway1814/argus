...
6. **Resolve Python once and record it as `python_cmd`.** Try `py -3`, `python3`, then `python`, each with `--version`, and keep the first that reports 3.9 or newer. On Windows a usable runtime frequently exists while neither `python` nor `py` is on PATH — if all three fail, look for `%LOCALAPPDATA%\Programs\Python\Python3*\python.exe` and store the absolute path. Every bundled script is then called as `<python_cmd> "${CLAUDE_SKILL_DIR}/<script>.py"`. Nothing found → say the one-line install (`winget install Python.Python.3.12` / `brew install python` / distro package); captions and metadata still work, but the frame, note, and Telegram helpers do not.
