...
2. Ears, transcribe the audio (faster-whisper, local, handles Hindi/Hinglish):
   ...
   ```
   If faster-whisper is unavailable or transcription exits nonzero, do not install anything. Continue with the post caption and frames, mark the audio unread, and avoid claims that depend on speech.
3. Eyes, reels are too short for scene detection, so ask for explicit stills every 2 seconds (`--stills 0,2,4,…` up to the reel's length):
   ...
   If ffmpeg or ffprobe is unavailable, or frame extraction exits nonzero, do not install anything. Continue from speech and the post caption, set `frames: no`, and state that burned-in overlays were not verified. Do not write a runbook step that depends on unread screen content.
...
Done when every available source has been read and the identity is in `videos/_processed.txt`. With frames, the runbook quotes what the reel showed, overlays included. Without frames, the note explicitly says overlays were not verified and makes no screen-dependent claim. If neither speech nor frames were readable and the post caption is insufficient, write the documented stub and do not report the reel as watched.
